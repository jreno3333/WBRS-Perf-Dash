...
export async function getHourlyBananaPuddingRates(targetDate: Date): Promise<Map<string, Record<number, { orders: number; hits: number; rate: number }>>> {
  const dateStr = targetDate.toISOString().split('T')[0];
  const fullStoreMap = await getCachedStoreMap();
  const storeToRestaurant = new Map<string, { id: string; timezone: string }>();
  fullStoreMap.forEach((info, storeNum) => {
  ...
  const bpCategory = ATTACHMENT_CATEGORIES['banana_pudding'];
  if (!bpCategory) return new Map();

  const result = new Map<string, Record<number, { orders: number; hits: number; rate: number }>>();
  ...
  for (const tz of timezoneSet) {
    ...
    for (const order of orders) {
      const info = storeToRestaurant.get(order.storeNumber);
      if (!info) continue;
      const rid = info.id;
      const h = order.hour;

      if (!result.has(rid)) result.set(rid, {});
      const hourMap = result.get(rid)!;
      if (!hourMap[h]) hourMap[h] = { orders: 0, hits: 0, rate: 0 };
      hourMap[h].orders++;

      const items = extractItemsFromOrder(order.rawJson || '');
      if (items.some(item => classifyItem(item, bpCategory))) {
        hourMap[h].hits++;
      }
    }
  }

  result.forEach((hourMap) => {
    for (const h of Object.keys(hourMap)) {
      const entry = hourMap[Number(h)];
      entry.rate = entry.orders > 0 ? Math.round((entry.hits / entry.orders) * 1000) / 10 : 0;
    }
  });
