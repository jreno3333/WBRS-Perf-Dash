...
- **Fiscal Calendar Ticker**: Dashboard "Total Sales" card includes a small badge (`Q3·P8 · Wk 1/4 · 9w left in Q3`) computed by `client/src/lib/fiscal-calendar.ts`. Calendar is 4-quarter × 3-period (4-4-5 weeks), business week Sat→Fri, fiscal year starts in October (anchor: FY2026 P1 = Saturday Oct 4, 2025; FY2026 Q3 P7 = Saturday Apr 4, 2026). Click the badge for a popover with full FY/Q/P start & end dates and weeks/days remaining in both the current period and quarter.
- **Sales Plan Upload**: Settings page accepts a multi-sheet `.xlsx` workbook (one sheet per unit, sheet name = unit number) with columns `location_code, business_date, gross_sales, comps_discounts, net_sales, paper_cost_pct, variable_labor_pct`. Stored in `sales_plan_daily` keyed by `(restaurant_id, date)` so it can drive plan-vs-actual variance on the dashboard. Endpoints: `POST /api/sales-plan/upload` (base64 xlsx), `GET /api/sales-plan/summary`, `GET /api/sales-plan?startDate&endDate&restaurantId`, `GET /api/sales-plan/qtd?date=YYYY-MM-DD` (per-unit QTD plan vs actual through the previous Central day, used for the leaderboard QTD row), `DELETE /api/sales-plan`. The leaderboard's labor row also reads `plannedVariableLaborPct` for each unit and uses it as the LC% target — Day LC% is graded against today's plan %, WTD LC% against the sales-weighted plan % across the current Sat→selected day window. Both fall back to the per-unit `restaurants.laborTarget` (default 25%) when no plan row exists. The backend exposes these as `dayPlanLaborPct` and `wtdPlanLaborPct` on the `/api/leaderboard` response, and `willHitLaborTarget` now uses the plan target when present.
- **Leaderboard QTD vs Plan**: Each leaderboard card shows a `QTD` row with quarter-to-date actual net sales (POS via `hourly_sales`) vs the uploaded sales plan for the same window (current fiscal quarter start → previous Central day, so today is excluded for apples-to-apples completed days). Variance is `(actual / plan − 1) × 100`, green ≥ 0 / red < 0; the row hides for units with no plan in the window. Hover shows the dollar variance and the date window. Replaces the previous trailing-14-day "2WK" row.
- **Attachment Rate Tracking**: Tracks 6 core upsell categories (cheese, bacon, jalapeños, dipping sauces, shakes & malts, whatasize) with benchmarks. Banana Pudding Shake and Kids Meal are tracked as separate promotional categories (each with a 5%/5-per-100-tickets target) without affecting the composite upsell score; units hitting target get a "Gone Bananas 🍌" badge and a "Kids Meal Crown 🧒" badge respectively. Both also appear as sort options in the leaderboard dropdown.
