...
export const restaurants = pgTable("restaurants", {
  ...
  openDate: date("open_date"), // Date when restaurant opened/will open. Future = training, < 120 days = new unit
...
export const posOrders = pgTable("pos_orders", {
  ...
  orderSource: text("order_source"), // Raw destination short_name: "dt1", "dt2", "dt3", "in", "app", "POS"
...
export const gradingConfig = pgTable("grading_config", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  config: jsonb("config").$type<GradingConfigData>().notNull(),
  updatedAt: timestamp("updated_at").defaultNow(),
  updatedBy: text("updated_by"),
});
